...
contract TimeTransfersLimitsModule is AbstractModuleUpgradeable {
    ...
    mapping(address => mapping(address => mapping(uint32 => TransferCounter)))
        public usersCounters;
    ...
    function setTimeTransferLimit(
        ...
        bool limitIsAttributed = limitValues[msg.sender][_limit.limitTime]
            .attributedLimit;
        ...
        if (!limitIsAttributed && limitCount < 4) {
            ...
            limitValues[msg.sender][_limit.limitTime] = IndexLimit(
                true,
                limitCount
            );
        } else {
            transferLimits[msg.sender][
                limitValues[msg.sender][_limit.limitTime].limitIndex
            ] = _limit;
        }
    ...
    function moduleCheck(
        ...
        if (!_getIdentity(_compliance, _from)) {
            // Use bool directly
            return false;
        }

        for (uint256 i = 0; i < transferLimits[_compliance].length; i++) {
            uint32 limitTime = transferLimits[_compliance][i].limitTime;
            uint256 limitValue = transferLimits[_compliance][i].limitValue;
            ...
            if (
                !_isUserCounterFinished(_compliance, _from, limitTime) &&
                usersCounters[_compliance][_from][limitTime].value + _value >
                limitValue
            ) {
                return false;
            }
    ...
    function _increaseCounters(
        ...
        require(
            _getIdentity(_compliance, _userAddress),
            "User is not a valid investor"
        );

        for (uint256 i = 0; i < transferLimits[_compliance].length; i++) {
            uint32 limitTime = transferLimits[_compliance][i].limitTime;

            _resetUserCounter(_compliance, _userAddress, limitTime);

            // Update counter
            usersCounters[_compliance][_userAddress][limitTime].value += _value;
        }
    }
     ...
     *  @param _compliance the Compliance smart contract address
     *  @param _userAddress  of user wallet
    ...
    function _resetUserCounter(
        ...
        if (_isUserCounterFinished(_compliance, _userAddress, _limitTime)) {
            TransferCounter storage counter = usersCounters[_compliance][
                _userAddress
            ][_limitTime];
     ...
     *  @param _compliance the Compliance smart contract to be checked
     *  @param _userAddress ONCHAINID of user wallet
     *  @param _limitTime limit time frame
     *  internal function, can be called only from the functions of the Compliance smart contract
     */
    function _isUserCounterFinished(
        ...
        return
            usersCounters[_compliance][_userAddress][_limitTime].timer <=
            block.timestamp;
    }
    ...
    function _getIdentity(
        ...
        return
            ISecurityToken(IModularCompliance(_compliance).getTokenBound())
                .identityStorage()
                .isValidInvestor(_userAddress);
    }
    ...
    function _isTokenAgent(
        ...
        return
            AgentRoleUpgradeable(IModularCompliance(compliance).getTokenBound())
                .isAgent(_userAddress);
    }
}