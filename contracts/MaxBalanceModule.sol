...
function moduleTransferAction(address _from, address _to, uint256 _value) external override onlyComplianceCall {
    require(_getIdentity(msg.sender, _from), "Invalid sender identity");
    require(_getIdentity(msg.sender, _to), "Invalid recipient identity");

    _IDBalance[msg.sender][_to] += _value;
    _IDBalance[msg.sender][_from] -= _value;

    if (_IDBalance[msg.sender][_to] > _maxBalance[msg.sender]) {
        revert MaxBalanceExceeded(msg.sender, _value);
    }
}
...
function moduleCheck(
    ...
    if (!_getIdentity(_compliance, _to)) {
        return false;
    }

    if ((_IDBalance[_compliance][_to] + _value) > _maxBalance[_compliance]) {
        return false;
    }
    ...
    *  @dev getter for compliance identity balance
    ...
    function canComplianceBind(address _compliance) external view returns (bool) {
        ...
        ISecurityToken token = ISecurityToken(IModularCompliance(_compliance).getTokenBound());
    ...
    function _getIdentity(address _compliance, address _userAddress) internal view returns (bool) {
        return ISecurityToken(IModularCompliance(_compliance).getTokenBound()).identityStorage().isValidInvestor(_userAddress);
    }
}
